...
中文首页昵称采用霞鹜文楷的真实 400 字重，避免大字号合成粗体；英文昵称保持 Maple Mono 700。英文叙述明确使用系统无衬线字体。首页组件的字号与间距不受文章正文默认规则覆盖。
...
角色仅在上缘破框，下缘使用连续圆形轮廓。入场结束后静止；精细指针可触发小幅跟随和柔和回位，说明文字固定。移动端使用约 216px 的角色舞台，不依赖悬停。

### Theme Transition

深色从主题按钮向外展开，浅色向按钮收拢；遮罩边缘轻微柔化，正文保持清晰。太阳与月亮使用独立的短过渡。连续点击遵循最后一次选择；减少动态模式和不支持 View Transitions 的浏览器即时切换。

### Embedded Blog

博客在站内窗口直接阅读，外层导航和简洁工具栏固定保留，滚动交给内嵌页面。原站链接始终可用，加载提示不遮挡操作。博客内容的主题由原站控件管理；跨域页面不注入样式或假设存在主题同步协议。
