...
- 角色插画：`gpt091-source.png` 用于首页，`gpt092-source.png` 用于关于页，`gpt093-source.png` 用于 404 页，原图位于 `scripts/assets/images/`。安装 libwebp（本次导出使用 cwebp 1.6.0）后，运行 `bash scripts/export_character_images.sh` 生成透明 WebP。脚本保留原图比例与内容，只调整分辨率和压缩；首页使用单层透明角色图，上方开放、底部弧形裁切，背景不随角色移动。
